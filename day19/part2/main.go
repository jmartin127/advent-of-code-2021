...
var orientedScanners = []*orientedScanner{}
...
func main() {
	...
	maxDistance := 0
	for i := 0; i < len(orientedScanners); i++ {
		for j := 0; j < i; j++ {
			dist := calcManhattanDistance(orientedScanners[i], orientedScanners[j])
			if dist > maxDistance {
				maxDistance = dist
			}
		}
...
func calcManhattanDistance(a, b *orientedScanner) int {
	return distCalc(a.position[0], b.position[0]) + distCalc(a.position[1], b.position[1]) + distCalc(a.position[2], b.position[2])
}

func distCalc(a, b int) int {
	...
	for rsPos, rs := range remainingScanners {
		for lsPos, ls := range lockedScanners {
			if hasOverlap, os, matchType := ls.scannersMatch(rs); hasOverlap {
				...
				orientedScanners = append(orientedScanners, os)
